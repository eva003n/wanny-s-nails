...
const log = logger.child({ module: "job:dead-letter" });
...
export function handleDeadLetterJob(queueName: string, job: Job | undefined, error: Error): void {
  ...
  if (deadLetterCounts[key] >= 10) {
    log.error(
      ...
      `ALERT: ${deadLetterCounts[key]} dead letter jobs in queue "${queueName}"`,
    );
  }
}
...
log.info({ event: "dead_letter.monitor.started" }, "Dead letter queue monitor started");