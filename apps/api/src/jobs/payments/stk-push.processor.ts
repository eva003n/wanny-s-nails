...
export const stkPushWorker = new Worker<StkPushJobData>(
  "payments",
  async (job: Job<StkPushJobData>) => {
...
stkPushWorker.on("failed", (job, err) => {
  log.error(
    { event: "stk_push.job.failed_permanently", jobId: job?.id, error: err.message },
    "STK Push job exhausted all retries",
  );
});

stkPushWorker.on("completed", (job) => {
  log.debug({ event: "stk_push.job.completed", jobId: job.id }, "STK Push job completed");
});

log.info({ event: "stk_push.worker.started" }, "STK Push worker started");