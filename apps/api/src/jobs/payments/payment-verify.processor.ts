...
export const paymentVerifyWorker = new Worker<PaymentVerifyJobData>(
  "payments",
  async (job: Job<PaymentVerifyJobData>) => {
    ...
    try {
      ...
      const response = await axios.post<DarajaQueryResponse>(
        config.DARAJA_STK_PUSH_URL.replace("/processrequest", "/query"),
        {
...
paymentVerifyWorker.on("failed", (job, err) => {
  log.error(
    { event: "payment_verify.job.failed_permanently", jobId: job?.id, error: err.message },
    "Payment verification job exhausted all retries",
  );
});

paymentVerifyWorker.on("completed", (job) => {
  log.debug({ event: "payment_verify.job.completed", jobId: job.id }, "Payment verification job completed");
});

log.info({ event: "payment_verify.worker.started" }, "Payment verification worker started");