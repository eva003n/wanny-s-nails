...
export const reminder24hWorker = new Worker<Reminder24hJobData>(
  "reminders",
  async (job: Job<Reminder24hJobData>) => {
    const { reminderId, bookingId, customerPhone, customerName, serviceName, appointmentAt } =
      job.data;

    log.info(
      { event: "reminder_24h.job.start", jobId: job.id, bookingId, customerPhone },
    ...
    try {
      ...
      log.error(
        { event: "reminder_24h.job.failed", jobId: job.id, bookingId, error: err.message },
...
reminder24hWorker.on("failed", (job, err) => {
  log.error(
    { event: "reminder_24h.job.failed_permanently", jobId: job?.id, error: err.message },
    "24h reminder job exhausted all retries",
  );
});

reminder24hWorker.on("completed", (job) => {
  log.debug({ event: "reminder_24h.job.completed", jobId: job.id }, "24h reminder job completed");
});

log.info({ event: "reminder_24h.worker.started" }, "24h reminder worker started");