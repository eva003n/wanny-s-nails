...
export const reminder1hWorker = new Worker<Reminder1hJobData>(
  "reminders",
  async (job: Job<Reminder1hJobData>) => {
    ...
    try {
      ...
      log.error(
        { event: "reminder_1h.job.failed", jobId: job.id, bookingId, error: err.message },
...
reminder1hWorker.on("failed", (job, err) => {
  log.error(
    { event: "reminder_1h.job.failed_permanently", jobId: job?.id, error: err.message },
    "1h reminder job exhausted all retries",
  );
});

reminder1hWorker.on("completed", (job) => {
  log.debug({ event: "reminder_1h.job.completed", jobId: job.id }, "1h reminder job completed");
});

log.info({ event: "reminder_1h.worker.started" }, "1h reminder worker started");