 ...
 */
export async function buildDateOptions(serviceId: string): Promise<DateOption[]> {
  ...
  while (options.length < 7 && attempts < maxAttempts) {
    ...
    try {
      // Import slotsService dynamically to avoid circular dependency
      const { slotsService } = await import("../modules/slots/slots.service.js");
      const slotData = await slotsService.getAvailableSlots(dateStr, serviceId);
      const availableCount = slotData.availableSlots;
      const isFull = availableCount === 0;

      const isToday =
        candidateDate.toISOString().split("T")[0] === today.toISOString().split("T")[0];
...
export async function findCustomerByPhone(
  ...
  return customersService.getByPhone(phone);
}
...
export async function findOrCreateCustomer(
  ...
  return customersService.findOrCreate(phone, profileName || "Customer");
}

/**
 * Find the latest active (non-cancelled, non-completed) booking for a customer.
 */
export async function findActiveBooking(customerId: string) {
