...
export  function createLogger(_config: Config) {
  ...
  if (isProduction) {
    ...
    targets.push({
      target: "@logtail/pino",
      options: {
        sourceToken: _config.LOGTAIL_SOURCE_TOKEN,
        options: {
          endpoint: _config.LOGTAIL_INGESTION_HOST as string,
        },
      },
    });
  }
  ...
  return pino(
    {
      ...
      base: { service: _config.APP_NAME, env: _config.NODE_ENV },
