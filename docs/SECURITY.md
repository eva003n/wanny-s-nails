...
4. Refresh token is hashed before DB storage (SHA-256)

**Token refresh:**
1. POST `/auth/refresh` with refreshToken
2. Server finds hashed token in DB, validates not expired/revoked
3. Issues new accessToken; optionally rotates refreshToken (sliding window)

**Logout:**
1. POST `/auth/logout` — marks refreshToken as revoked in DB
2. PWA app deletes both tokens from Keychain
...
| Role | Permissions |
|---|---|
| `OWNER` | All operations including: delete service, manage team, view all reports, mark manual payments |
...
// Usage
router.delete('/services/:id', authenticate, requireRole(['OWNER']), deleteService);
```
...
**Rotation policy:**
- JWT_SECRET: rotate every 90 days (all active sessions invalidated — acceptable given 30-day refresh token TTL with re-login prompt)
